...
export default function GPUManagementPage() {
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      console.error("EKRAN KARTI YÜKLEME HATASI:", error);

      alert(
        "Ekran kartları yüklenemedi.\n\n" +
  ...
  const openEditModal = (item: HardwareItem) => {
    ...
    setForm({
      ...
      vram: getSpec(item.specs, [
        "VRAM",
        "vram",
      ]),

      memoryType: getSpec(item.specs, [
        "Bellek Türü",
        "bellek türü",
        "Bellek",
        "bellek",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("EKRAN KARTI KAYDETME HATASI:", error);

      alert(
        "Ekran kartı kaydedilemedi.\n\n" +
          (error?.message || JSON.stringify(error))
      );
    } finally {
      setSaving(false);
  ...
  const handleDelete = async (item: HardwareItem) => {
    const confirmed = window.confirm(
      `"${item.name}" ekran kartını silmek istediğine emin misin?`
    );
    ...
    try {
      ...
      console.error("EKRAN KARTI SİLME HATASI:", error);

      alert(
        "Ekran kartı silinemedi.\n\n" +
          (error?.message || JSON.stringify(error))
      );
    }
  };
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-500 mt-1">
                GPU ekle, düzenle ve mevcut ekran kartlarını yönet.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 border-b border-zinc-800">
            ...
            <div>
              <h2 className="text-sm font-black">
                Kayıtlı Ekran Kartları
              </h2>

              <p className="text-[11px] text-zinc-500 mt-1">
                Toplam {items.length} ekran kartı
              </p>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Örn: NVIDIA GeForce RTX 4070"
