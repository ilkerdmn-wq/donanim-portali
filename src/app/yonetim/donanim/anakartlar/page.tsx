...
export default function MotherboardManagementPage() {
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      console.error("ANAKART YÜKLEME HATASI:", error);

      alert(
        "Anakartlar yüklenemedi.\n\n" +
  ...
  const filteredItems = useMemo(() => {
    ...
    const filtered = !q
      ...
      : items.filter((item) => {
          ...
          const chipset = getSpec(item.specs, [
            "Yonga Seti",
            "yonga seti",
            "Chipset",
            "chipset",
  ...
  const handleSave = async () => {
    ...
    const specs = {
      ...
      "Yonga Seti": form.chipset.trim(),
      Bellek: form.memory.trim(),
    ...
    try {
      ...
      console.error("ANAKART KAYDETME HATASI:", error);

      alert(
        "Anakart kaydedilemedi.\n\n" +
          (error?.message || JSON.stringify(error))
      );
    } finally {
      setSaving(false);
  ...
  const handleDelete = async (item: HardwareItem) => {
    const confirmed = window.confirm(
      `"${item.name}" anakartını silmek istediğine emin misin?`
    );
    ...
    try {
      ...
      console.error("ANAKART SİLME HATASI:", error);
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-500 mt-1">
                Anakart ekle, düzenle ve mevcut
                anakartları yönet.
              </p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div>
              {filteredItems.map((item) => {
                ...
                const memory = getSpec(item.specs, [
                  "Bellek",
                  "bellek",
                  "RAM Tipi",
                  "ram tipi",
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Örn: MSI B650 Gaming Plus WiFi"
