...
export default function İncelemeYonetimiPage() {
  ...
  const loadNews = async () => {
    ...
    try {
      ...
      setNews(result.reviews || []);
    } catch(error) { alert(error instanceof Error ? error.message : "İncelemeler yüklenemedi."); }
  ...
  const resetForm = () => {
    ...
    setEditingNews(null);
    setTitle(""); setSubtitle(""); setSku(""); setSpecs({});
    setExcerpt("");
    setCategory("Laptop");
    setSlug("");
    setPublished(false);
  ...
  const openEditEditor = (item: NewsItem) => {
    ...
    setEditingNews(item);
    setTitle(item.title || ""); setSubtitle(item.subtitle || ""); setSku(item.sku || ""); setSpecs(item.specs || {});
    setExcerpt(item.excerpt || "");
    setCategory(item.category || "Laptop");
  ...
  const uploadImage = async (file: File, folder: string) => {
    ...
    const response = await fetch("/api/admin/reviews/upload", {method:"POST",body:form});
    const result = await response.json();
  ...
  const validateForm = () => {
    if (!title.trim()) {
      alert("İnceleme başlığı boş bırakılamaz.");
      return false;
    }

    if (!slug.trim()) {
      alert("İnceleme adresi / slug boş bırakılamaz.");
      return false;
    }
    ...
    if (
      ...
      alert("İncelemeye en az bir içerik eklemelisin.");
      return false;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        title: title.trim(),
        subtitle: subtitle.trim(), sku: sku.trim(), specs,
        slug: finalSlug,
      ...
      const response = await fetch("/api/admin/reviews", {method:editingNews ? "PATCH":"POST", headers:{"Content-Type":"application/json"},body:JSON.stringify({...payload,id:editingNews?.id})});
      const result = await response.json();
      if(!response.ok) throw new Error(result.error || "Kayıt başarısız.");
      alert("İnceleme kaydedildi.");
      setEditorOpen(false);
      resetForm();
      await loadNews();
    } catch (error: any) {
      console.error("İNCELEME KAYDETME HATASI:", error);
      alert(
  ...
  const handleDelete = async (item: NewsItem) => {
    const accepted = window.confirm(
      `"${item.title}" incelemesini silmek istediğine emin misin?`
    );

    if (!accepted) return;

    try {
      const response = await fetch(`/api/admin/reviews?id=${item.id}`,{method:"DELETE"});
      const result = await response.json(); if(!response.ok) throw new Error(result.error);
      await loadNews();
    } catch(error) { alert(error instanceof Error ? error.message : "Silinemedi."); }
  };
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h1 className="text-3xl font-black">İnceleme Yönetimi</h1>
                  <p className="text-xs text-zinc-500 mt-1">
                    İnceleme ekle, düzenle, yayınla ve portal içeriğini yönet.
                  </p>
                </div>
              </div>
            ...
            <div className="px-5 md:px-6 py-5 border-b border-zinc-800 flex items-center justify-between">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-cyan-400">
                  {editingNews ? "İncelemeyi Düzenle" : "Yeni İçerik"}
                </p>
                <h2 className="text-xl font-black mt-1">
                  {editingNews ? editingNews.title : "Yeni İnceleme Oluştur"}
                </h2>
              </div>
                ...
                <div className="space-y-7">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="İnceleme başlığını yaz..."
                  ...
                  <div>
                    <label htmlFor="review-subtitle" className="block text-[11px] font-black uppercase text-zinc-400 mb-2">Alt Başlık</label>
                    <input id="review-subtitle" value={subtitle} maxLength={300} onChange={e=>setSubtitle(e.target.value)} placeholder="Başlığın altında görünecek alt başlık..." className="w-full h-14 rounded-xl border border-zinc-800 bg-zinc-950 px-4 text-sm outline-none focus:border-cyan-500/50" />
                  </div>
                  <div>
                    <label className="flex items-center gap-2 text-[11px] font-black uppercase text-zinc-400 mb-2">
                      <AlignLeft size={13} />
                    ...
                    <textarea
                      ...
                      placeholder="Ana sayfa ve inceleme kartlarında görünecek kısa açıklama..."
                      rows={4}
              ...
              <aside className="lg:col-span-4 p-5 md:p-7 bg-zinc-950/30">
                <div className="lg:sticky lg:top-24 space-y-6">
                  <div>
                    ...
                    <h3 className="text-lg font-black mt-1">İnceleme Bilgileri</h3>
                  </div>
                  ...
                  <div>
                    <label className="flex items-center gap-2 text-[10px] font-black uppercase text-zinc-500 mb-2">
                      <Link2 size={12} />
                      İnceleme Adresi / Slug
                    </label>

                    <input
                      value={slug}
                      onChange={(e) => setSlug(createSlug(e.target.value))}
                      placeholder="inceleme-adresi"
                    ...
                    <p className="text-[9px] text-zinc-700 mt-2 break-all">
                      /incelemeler/{slug || "inceleme-basligi"}
                    </p>
                  </div>
                  ...
                  <div className="rounded-2xl border border-zinc-800 bg-zinc-950 overflow-hidden">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-[10px] text-zinc-600 mt-0.5">
                            {published
                              ? "İnceleme ziyaretçilere açık."
                              : "İnceleme taslak olarak kalır."}
                          </p>
                        </div>
                      </div>
                    ...
                    <details open className="rounded-2xl border border-zinc-800 bg-zinc-950 p-4">
                      <summary className="cursor-pointer text-xs font-black text-cyan-400">Teknik Özellikler</summary>
                      <div className="mt-4 space-y-3">{SPEC_FIELDS.map(([key,label])=><label key={key} className="block text-xs text-zinc-400">{label}<input value={specs[key] || ""} maxLength={180} onChange={e=>setSpecs({...specs,[key]:e.target.value})} className="mt-1 w-full rounded-lg border border-zinc-800 bg-zinc-900 px-3 py-2 text-zinc-200" /></label>)}
                      <label className="block text-xs text-zinc-400">Model / SKU<input value={sku} maxLength={120} onChange={e=>setSku(e.target.value)} className="mt-1 w-full rounded-lg border border-zinc-800 bg-zinc-900 px-3 py-2" /></label></div>
                    </details>
        ...
        {!editorOpen && (
          <section className="rounded-3xl border border-zinc-800 bg-zinc-900/40 overflow-hidden">
            <div className="px-5 py-4 border-b border-zinc-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h2 className="text-sm font-black">Kayıtlı İncelemeler</h2>
                <p className="text-[10px] text-zinc-600 mt-1">
                  Toplam {news.length} inceleme
                </p>
              </div>
            ...
            <div className="max-w-[900px] mx-auto">
              ...
              <article className="rounded-3xl border border-zinc-800 bg-zinc-950 p-6 md:p-10">
                ...
                <div className="mt-8 pt-8 border-t border-zinc-800 space-y-6">
                  <p className="mb-4 text-lg text-zinc-400">{subtitle}</p><ReviewSpecs specs={specs} />
