...
export type ComparisonColumn = { id: string; name: string; reviewSlug: string; imageUrl: string };
...
export const featureLabels = ["İşlemci", "Harici Grafik İşlemcisi (GPU)", "RAM kapasitesi", "RAM hızı", "RAM yükseltme", "Ekran", "Yenileme hızı", "Parlaklık", "Renk kapsamı", "SSD", "Dahili grafik", "Thunderbolt 4", "Wi-Fi", "Bluetooth", "Pil kapasitesi", "Ağırlık", "Kalınlık", "Kamera çözünürlüğü"];

export function emptyComparison(): ManualComparison {
  const columns = [{id:"laptop-1",name:"",reviewSlug:"",imageUrl:""},{id:"laptop-2",name:"",reviewSlug:"",imageUrl:""}];
...
export function normalizeComparison(value: unknown): ManualComparison {
  ...
  const image = (v:unknown) => {const url=trim(v,1000);if(!url)return "";if(url.startsWith("/")&&!url.startsWith("//"))return url;try{return new URL(url).protocol==="https:"?url:""}catch{return ""}};
  const columns = input.columns.map((item,index)=>{const col=item as Record<string,unknown>;return {id:`laptop-${index+1}`,name:trim(col?.name,180),reviewSlug:trim(col?.reviewSlug,220),imageUrl:image(col?.imageUrl)};});
